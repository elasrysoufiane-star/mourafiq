...
**Prompts accessibilité (`src/ai/claude_client.py`) :** quatre prompts système dédiés —
`_VISION_SYSTEM_PROMPT` (sécurité d'abord « عندك! » ; **nommer les objets précisément** — jamais « شي حاجة » ; **JAMAIS de couleurs** — inutiles pour un non-voyant, sauf feu rouge/vert ; position+distance ; texte visible lu ; court), `_CHAT_SYSTEM_PROMPT` (compagnon darija patient), `_OCR_SYSTEM_PROMPT` (lit + donne le sens : courrier, médicaments, panneaux), `_INTENT_SYSTEM_PROMPT` (classification des transcriptions STT déformées). Couvre rue + intérieur + lecture.
...
| TTS propre : phrases tronquées coupées + OCR muet fiable (2026-07-11) | Log de répétition : (a) les réponses scène plafonnées par max_tokens tombaient en plein mot (« خلفو جدار أ ») → le TTS lisait des phrases cassées ; (b) l'OCR sans texte bavardait (décrivait la chambre AVEC couleurs, posait des questions bizarres) et une variante de formulation (« ماكاينش فيها حتى نص ») échappait au filtre → parlée à chaque cycle. Fix : (1) `text_clean.couper_phrase_incomplete()` — coupe à la dernière phrase complète (., !, ؟, …), appliquée aux deux sorties de la boucle auto ; (2) `_OCR_SYSTEM_PROMPT` durci : sans texte → la phrase sentinelle EXACTE seule, jamais de description de scène/couleurs/questions ; (3) `_OCR_SANS_RESULTAT` étendu à 8 variantes observées. Testé (`test_coupe_*`) | `src/audio/text_clean.py`, `src/ai/claude_client.py`, `src/vision/detector.py`, `tests/test_memory.py` |
| Prompt vision : objets nommés, zéro couleur (2026-07-11) | Logs réels : descriptions pleines de couleurs (« صندوق أزرق كبير ») et de « شي حاجة » vagues. Pour un NON-VOYANT les couleurs sont du bruit qui gaspille le budget parlé, et « des choses » n'informe pas. `_VISION_SYSTEM_PROMPT` réécrit : (règle 2) NOMMER chaque objet précisément (طاولة، شاشة، طابعة...), interdiction de « شي حاجة » ; (règle 3) JAMAIS de couleurs — exception unique : le feu rouge/vert (l'info EST la couleur) ; (règle 5) si un texte est visible et lisible (lafita, plaque, boîte), dire ce qui est écrit. S'applique à la boucle continue ET à la demande (même prompt) | `src/ai/claude_client.py` |
