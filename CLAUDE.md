...
| `CLAUDE_TEXT_MODEL` | `claude-sonnet-5` | Modèle conversation darija (`claude_darija`) — multimodal (suivi visuel) |
| `CLAUDE_VISION_MODEL` | `claude-sonnet-5` | Modèle VLM **continu** (boucle auto + OCR de fond) — Sonnet 5 (qualité, latence OK en fond) |
| `CLAUDE_VISION_MODEL_HQ` | `claude-sonnet-5` | Modèle VLM scène **à la demande** (« شنو قدامي ») |
...
| `CLAUDE_SCENE_AUTO_MAX_TOKENS` | `80` | Plafond scène **boucle de fond** — court (~8s parlé, moins d'écho/chevauchement) |
...
| `AUTO_DESCRIBE_INTERVAL` | `10` | Description scène (toujours Claude) + OCR TOUJOURS actives, avec ou sans micro (s ; 0 = désactivé). 10s laisse un silence pour écouter l'utilisateur. ≈360 appels scène/h, ×2 avec OCR claude |
